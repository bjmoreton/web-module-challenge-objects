...
const testOne = createMenuItem("Test 1", 5, "Lunch");
const testTwo = createMenuItem("Test 2", 10, "Dinner");
const testThree = createMenuItem("Test 3", 8, "Breakfast");
...
function getLastReview(objects) {
  ...
  console.log(`${review.name} gave the restaurant a ${review.rating}, and their feedback was: ${review.feedback}`);
}
getLastReview();
...
function getReviewByRating(objects, rating) {
  let returnArray = [];
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].rating >= rating && objects[i].rating < rating + 1) returnArray.push(objects[i]);
  }
...
and should return an array of objects. 
  ...
  For example, invoking getLongReviews(reviews) would return [
    ...
    {name: "Julius", rating: 2, feedback: "I was largely unimpressed by this venue. Nothing special on the menu and too expensive. The atmosphere is polarizing, and not for me, but I think some would like it." }]
*/
function getLongReviews(objects) {
  let returnArray = [];

  for (let i = 0; i < objects.length; i++) {
    let review = objects[i];

    if (review.feedback.split(" ").length > 15) returnArray.push(objects[i]);
  }
